...
```bash
uv run python scripts/dump_tools.py tests/snapshots/live_tools.json "$(cygpath -w "$PWD/.venv/Scripts/python.exe")" -c "from word_document_server.main import run_server; run_server()"
```
